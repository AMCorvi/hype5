...
function casperjsFunction(type, filter) {
  return new Promise(function (resolve, reject) {
    ...
    if (type && filter) {
      exec("casperjs --verbose " + path.join(__dirname, "./crawler.js") + " --hypeType=" + type + " --hypeFilter=" + filter, data);
    } else if (type) {
      exec(" casperjs --verbose ./crawler.js " + path.join(__dirname, "./crawler.js") + " --hypeType=" + type + " --hypeFilter=" + filter, data);
    } else if (!type) {
...
function validate(type, filter) {
  ...
  var typeIsValid = type === ("popular" || "latest");
  if (!typeIsValid) {
