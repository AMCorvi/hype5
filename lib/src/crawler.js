...
var opt = function () {
  var options = {};
  casper.cli.has("hypeType") ? options.type = casper.cli.get("hypeType") : null;
  casper.cli.has("hypeFilter") ? options.filter = casper.cli.get("hypeFilter") : null;
  return options;
}();
...
function getTrackInfo(tracks) {
  ...
  var intro = Array.prototype.map.call(element, function (element) {
    try {
      ...
      var descFunc = function descFunc() {
...
function scanHype(type, filter) {
  ...
  var url = filter ? "http://hypem.com/" + type + "/" + filter : "http://hypem.com/" + type;
